...
public class Parser {
    ...
    public IR.Address F() {
        ...
        if (attr == '(') {
            ...
            match(Token.CONST_INT);
            IR.Address F_addr_syn = new IR.TempAddress(new_temp());
            gen(IR.ASSIGN, new IR.ConstInteger(Integer.parseInt(token.getValue())), null, F_addr_syn);
            return F_addr_syn;
        }
            ...
            IR.Address F_addr_syn = new IR.TempAddress(new_temp());
            gen(IR.ASSIGN, new IR.ConstReal(Double.parseDouble(token.getValue())), null, F_addr_syn);
            return F_addr_syn;
        }
        else {
            Token id = tokens.get(ptr);
    ...
    public IR.Address L_(Type type) {
        ...
        if (attr == '[') {
            ...
            IR.Address L__offset = new IR.TempAddress(new_temp());
            gen(IR.ASSIGN, new IR.ConstInteger(0), null, L__offset);
            return L__offset;
        }
    }
